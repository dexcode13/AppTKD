<?php 
/*
* Archivo solicitudes-view.php
* Contiene las vistas de mostrar Solicitudes, nueva Solicitud y editar Solicitud.
...
<div class="row">
	...
	<h2><i class='fa fa-archive'></i> Expediente de Alumnos</h2>
	</div>
    ...
    //Si es admin
    if($p->id_tipo==1){
      $request = SolicitudData::getAllAlumnos();
		if(count($request)>0){
			?>
...
<div class="box-body">
      ...
      <th>Correo</th>
			...
			foreach($request as $request1){
				...
				<tr>
        <td>
          <a href='./?view=editAlumno&id=<?php echo $request1->rgi;?>'><?php echo $request1->rgi; ?></a>
        </td>
        <td><?php echo $request1->nombre." ".$request1->apellido;  if ($request1->ingreso==2019) {
											$fecha = date('Y-m-d');
											$fechainicial = new DateTime($request1->fecha_ingreso);
											$fechafinal = new DateTime($fecha);
											$diferencia = $fechainicial->diff($fechafinal);
											//$meses = ( $diferencia->y * 12 ) + $diferencia->m;
                       echo " <span class='label label-success label-as-badge'>".$diferencia->format('%R%a días')."</span>";
                      } ?></td>
        ...
        <td><?php echo $request1->correo; ?></td>
		...
		$request = SolicitudData::getByGerente();
		if(count($request)>0){
			?>
<div class="box box-primary">
<div class="box-body">
<div class="table-responsive">
      ...
      <th>Pagada</th>
			...
			foreach($request as $request1){
				?>
				<tr>
				<td><a href='./?view=opensolicVobo&id=<?php echo $request1->id_solicitud;?>'><i class="fa fa-eye fa-fw"></i><?php echo $request1->id_solicitud; ?></a></td>
     ...
     <td style="width:100px;"><center>
		 ...
		 <td><center>
				<button type="button" class="btn btn-social-icon btn-success" title="Imprimir" id="view" onclick="window.open('/AppViatics.4.0/core/app/view/formato-solicitud.php?&id=<?php echo $request1->id_solicitud; ?>','_blank')"><i class=" fa fa-print"></i></button></center>
		...
		//Si es usuario y aux contable
    else if ($p->id_tipo==2 and $p->id_perfil==10) {
			$request = SolicitudData::getAllAprobadasAux();
if(count($request)>0){
	?>
<div class="box box-primary">
<div class="box-body">
...
echo $request1->fecha_status;}
 ...
 <td><center> 
<button type="button" class="btn btn-social-icon btn-success" title="Imprimir" id="view" onclick="window.open('/AppViatics.4.0/core/app/view/formato-solicitud.php?&id=<?php echo $request1->id_solicitud; ?>','_blank')"><i class=" fa fa-print"></i></button>
 </center>
 </td>
 <td style="width:100px;"><center><?php  
		if($request1->comprobacion) { echo "<p class='label label-success'><i class='glyphicon glyphicon-ok'></i> Comprobado</p><br>";
	...
	echo "<p class='alert alert-danger'>No hay Solicitudes</p>";
		...
		//Si es usuario y Tesorero
      		...
      		$request = SolicitudData::getAllAprobadas();
		if(count($request)>0){
			?>
<div class="box box-primary">
<div class="box-body">
<div class="table-responsive">
      ...
      <th>Pago</th>
			...
			foreach($request as $request1){
				?>
				<tr>
				<td><a href='./?view=opensolicPago&id=<?php echo $request1->id_solicitud;?>'><i class="fa fa-eye fa-fw"></i><?php echo $request1->id_solicitud; ?></a></td>
     ...
     <td style="width:100px;"><center>
		...
		//Si es usuario y juridico
      		...
      		$request = SolicitudData::getAllByUser();
		if(count($request)>0){
			?>
<div class="box box-primary">
<div class="box-body">
<div class="table-responsive">
      ...
      <th>Acción</th>
        ...
        {
          ?><a href='./?view=opensolicitudCan&id=<?php echo $request1->id_solicitud;?>'><i class="fa fa-eye fa-fw"></i><?php echo $request1->id_solicitud; ?></a><?php
        }else if($request1->id_status==3)
        {
          ?><a href='./?view=opensolicitudPen&id=<?php echo $request1->id_solicitud;?>'><i class="fa fa-eye fa-fw"></i><?php echo $request1->id_solicitud; ?></a><?php
        }
				else
				{
