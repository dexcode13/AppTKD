...
$r = new AlumnosData();
$r->fecha_PagoAnualidad = $_POST["fecha"];
$r->rgi = $_POST["rgi"];
$r->id_control = $_POST["espe"];
$r->importe = $_POST["pago"];
$r->name = $_POST["nombre"];
$r->ciclo = $_POST["ciclo"];
$year = date('ymdhis');
$r->folio = $year;
$r->updateAnualidadGIM();
...
/**/
	...
	# Para mostrar el total
		...
		$printer->text("FECHA:".$r->fecha_PagoAnualidad. "\n");
		$printer->text("RGI:".$r->rgi. "\n");
		$printer->text("ALUMNO:".$newtext."\n");
		$printer->text("ANUALIDAD:".$r->ciclo. "\n");
	...
	$printer->close();
	Core::alert("Anualidad PAGADA satisfactoriamente");
Core::redir("./index.php?view=cobrosAnualidadGIM");
