...
class ProjectData {
	...
	public function add(){
		...
		$sql .= "value (\"$this->razon\",\"$this->rfc\",\"$this->direccion\",\"$this->cp\",\"$this->correo\",\"$this->telefono\")";
		Executor::doit($sql);
	}
...
// partiendo de que ya tenemos creado un objecto ProjectData previamente utilizamos el contexto
	...
	public function update(){
		$sql = "update ".self::$tablename." set name=\"$this->razon\",rfc=\"$this->rfc\",direccion=\"$this->direccion\",cp=\"$this->cp\",correo=\"$this->correo\",telefono=\"$this->telefono\" where id=$this->id";
		Executor::doit($sql);
	}
